...
詳細については、*Audience Manager ユーザーガイド*&#x200B;の[宛先](https://docs.adobe.com/content/help/ja-JP/audience-manager/user-guide/features/destinations/destinations.html)を参照してください。
...
データソース ID（[!UICONTROL DPID]）がドロップダウンリストに表示されない場合は、[データソースの設定ページ](https://docs.adobe.com/content/help/ja-JP/audience-manager/user-guide/features/data-sources/manage-datasources.html)のデータソースレベルで、**[!UICONTROL Outbound]** チェックボックスを選択する必要があります。
...
* **[!UICONTROL Sync Type]**：（必須）この宛先で使用する同期タイプを選択します。これは、クライアントが送信オーダーに含めるユーザーアクティビティのレベルを表します。クライアントがプロパティからのセグメント認定のみをおこなう場合は、「**[!UICONTROL Customer]」を選択します。**&#x200B;すべての **ユーザーにまたがるオフサイトアクティビティからのセグメント認定を含める場合は、「[!UICONTROL Platform]**[!DNL Audience Manager]」を選択します。
...
1. 目的の宛先の「**[!UICONTROL Actions]**」列で「![](assets/icon_delete.png)」をクリックします。
